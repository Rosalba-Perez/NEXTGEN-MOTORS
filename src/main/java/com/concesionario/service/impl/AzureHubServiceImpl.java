...
import com.concesionario.service.interfaces.IAzureHubService;
...
@Service
public class AzureHubServiceImpl implements IAzureHubService {
    ...
    public AzureHubServiceImpl(WebClient.Builder webClientBuilder) {
    ...
    public void sendNotification(String jsonBody, String tag) {
        try {
            ...
            // Prepare URL
            // Format:
            // https://<namespace>.servicebus.windows.net/<hub-name>/messages/?api-version=2015-01
            ...
            // Send Request
            // Header "ServiceBusNotification-Format": "gcm" for FCM v1/Legacy (Azure uses
            // 'gcm' for Google/Firebase)
            // Header "ServiceBusNotification-Tags": tag (if targeting specific user logic)
            // For Direct Send (no registration), we use
            // "ServiceBusNotification-DeviceHandle" header if targeting a token directly.

            // However, the standard "Send" API requires a tag or uses broadcast.
            // If we want to send to a SPECIFIC DEVICE TOKEN (Direct Send), we use the
            // specific header:
    ...
    private String generateSasToken(String resourceUri, String keyName, String key) {
        try {
            ...
            return "SharedAccessSignature sr=" + encodedUri + "&sig=" + encodedSignature + "&se=" + epoch + "&skn="
                    + keyName;
        } catch (Exception e) {
