...
@Service
public class GeminiAIServiceImpl implements IGeminiAIService {
    ...
    public Mono<String> analizarYSeleccionarVehiculo(String mensajeUsuario, List<Vehiculo> vehiculosDisponibles) {
        ...
        if (vehiculosDisponibles.isEmpty()) {
            return Mono.just(
                    "🤖 **Dante**: Lo siento, no tengo vehículos disponibles en este momento. Por favor contacta a un asesor humano.");
        }

        String infoVehiculos = construirInfoVehiculos(vehiculosDisponibles);

        // ✅ DETECTAR INTENCIÓN BÁSICA
        boolean esAlternativa = mensajeUsuario.toLowerCase()
                .matches(".*(otro|otra|diferente|más|mas|opción|opcion|ver más).*");
        ...
        return webClient.post()
                ...
                .onErrorResume(e -> {
                    ...
                    return Mono.just(
                            "🤖 **Dante**: Tuve un pequeño problema técnico pensando tu respuesta. ¿Me das un segundo y me repites qué buscabas?");
                });
    }

    private String construirInfoVehiculos(List<Vehiculo> vehiculos) {
        StringBuilder sb = new StringBuilder();
        int maxVehiculos = Math.min(vehiculos.size(), 25); // Limitar contexto para no exceder tokens gratis si hay
                                                           // muchos
