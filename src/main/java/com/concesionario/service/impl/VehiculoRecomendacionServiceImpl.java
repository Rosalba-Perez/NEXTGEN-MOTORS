...
import com.concesionario.service.interfaces.IGeminiAIService;
...
@Service
public class VehiculoRecomendacionServiceImpl implements IVehiculoRecomendacionService {

    private final IVehiculoService vehiculoService;
    private final IGeminiAIService geminiAIService;
    ...
    @Autowired
    public VehiculoRecomendacionServiceImpl(IVehiculoService vehiculoService,
    ...
    public RecomendacionResponse procesarRecomendacion(String mensajeUsuario) {
        try {
            ...
            // geminiAIService.analizarYSeleccionarVehiculo(mensajeUsuario,
            // vehiculosFiltrados).block();
            ...
            List<Vehiculo> vehiculoUnico = vehiculoSeleccionado != null ? List.of(vehiculoSeleccionado) : List.of();
     ...
     * En lugar de filtrar estrictamente, pasamos los vehículos al LLM para que él
     * decida.
    ...
    private List<Vehiculo> filtrarVehiculosBasico(List<Vehiculo> vehiculos, String mensaje) {
        // Para máxima "inteligencia", le damos al modelo casi todo y dejamos que él
        // razone.
        // Solo filtramos si la lista es enorme, pero aquí priorizamos la capacidad
        // semántica de Gemini.

        // Si hay más de 20 vehículos, hacemos un filtro ligero por coincidencia de
        // texto muy básica
    ...
    private boolean coincideCategoria(Vehiculo vehiculo, String mensajeLower) {
        ...
        if (mensajeLower.contains("camioneta")
                && (categoriaLower.contains("suv") || categoriaLower.contains("pick-up"))) {
